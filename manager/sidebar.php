...
<div class="sidebar">
    ...
    <a href="#appointments"><i class="fas fa-calendar-alt"></i> Users</a>
    ...
    <div class="content">
        ...
        <div class="services-grid">
            <div class="service-card">
                <h3><i class="nav-icon"><img src="../assets/fonts/member.png" alt="greet"></i> Total Clients</h3><hr>
                <?php
                $sql_clients = "SELECT COUNT(*) as total_clients FROM users WHERE user_id = ?";
                ...
                echo "<h3 style='text-align:center; '>" . $total_clients . "</h3>";

                $stmt_clients->close();
                ?>
            </div>
            <div class="service-card">
                <h3><i class="nav-icon"><img src="../assets/fonts/case.png" alt="greet"></i> Cases Registered</h3><hr>
                <?php
                $sql_cases = "SELECT COUNT(*) as total_cases FROM cases WHERE user_id = ?";
                $stmt_cases = $conn->prepare($sql_cases);
                $stmt_cases->bind_param("i", $user_id);
                $stmt_cases->execute();
                $result_cases = $stmt_cases->get_result();
                $row_cases = $result_cases->fetch_assoc();
                $total_cases = $row_cases['total_cases'];
                echo "<h3 style='text-align:center; '>" . $total_cases . "</h3>";
                $stmt_cases->close();
                ?>
            </div>
            <div class="service-card">
                <h3><i class="nav-icon"><img src="../assets/fonts/user.png" alt="greet"></i> Total Lawyers</h3><hr>
                <?php
                $sql_lawyers = "SELECT COUNT(*) as total_lawyers FROM users WHERE user_id = ?";
                $stmt_lawyers = $conn->prepare($sql_lawyers);
                $stmt_lawyers->bind_param("i", $user_id);
                $stmt_lawyers->execute();
                $result_lawyers = $stmt_lawyers->get_result();
                $row_lawyers = $result_lawyers->fetch_assoc();
                $total_lawyers = $row_lawyers['total_lawyers'];
                echo "<h3 style='text-align:center; '>" . $total_lawyers . "</h3>";
                $stmt_lawyers->close();
                ?>
            </div>
        </div>
